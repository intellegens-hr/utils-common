...
namespace Intellegens.Commons.Db.Extensions
{
    public static class TrackingDbContextExtensions
    {
        ...
        public static List<Type> GetDbContextBaseEntityTypesCached<T>(this T dbContext)
            where T : TrackingDbContextAbstract
        {
            ...
            if (!typesCache.ContainsKey(dbContextType))
            {
                var props = dbContext
                    ...
                    .Where(x => typeof(BaseEntityAbstract).IsAssignableFrom(x))
                    .ToList();
        ...
        public static void SetGlobalQueryFilter<T>(this T dbContext, ModelBuilder modelBuilder, string baseEntityPropertyName, object ExpectedValue, ComparisonTypes comparisonType = ComparisonTypes.EQUAL)
            where T : TrackingDbContextAbstract
        {
            foreach (var type in dbContext.GetDbContextBaseEntityTypesCached())
            {
