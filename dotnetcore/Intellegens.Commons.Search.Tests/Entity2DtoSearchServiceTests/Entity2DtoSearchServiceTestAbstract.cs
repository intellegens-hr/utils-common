...
using Intellegens.Commons.Search.Tests.Entity2DtoSearchServiceTests.Setup;
    ...
    public abstract class Entity2DtoSearchServiceTestAbstract
    {
        ...
        public async Task Fulltext_search_should_work_with_any_class()
        {
            ...
            var expectedCount = query
                .ToList()
                .Where(x =>
                    x.Text.Contains(textToSearch, StringComparison.InvariantCultureIgnoreCase)
                    || x.TestingSessionId.Contains(textToSearch, StringComparison.InvariantCultureIgnoreCase)
                    || (x.Sibling?.Text.Contains(textToSearch, StringComparison.InvariantCultureIgnoreCase) ?? false)
                    || (x.Children?.Any(c => c.Text != null && c.Text.Contains(textToSearch, StringComparison.InvariantCultureIgnoreCase)) ?? false)
                )
                .Count();
            var data = await searchService.Search(query, searchRequest);
